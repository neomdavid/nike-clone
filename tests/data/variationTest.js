import { updateClickedVariation, loadFromStorage, clickedVariation, getClickedVariation} from "../../scripts/data/variation.js";
...
describe('test suite: getClickedVariation',()=>{
  beforeEach(()=>{
    spyOn(localStorage, 'setItem');
  });

  it('gets existing clicked variation',()=>{
    spyOn(localStorage, 'getItem').and.callFake(()=>{
      return JSON.stringify(variation1);
    });

  loadFromStorage();
  expect(getClickedVariation()).toEqual(variation1);
  expect(localStorage.setItem).toHaveBeenCalledTimes(0);
  expect(localStorage.getItem).toHaveBeenCalledWith('clickedVariation');
  });

  it('gets not existing clicked variation',()=>{
    spyOn(localStorage, 'getItem').and.callFake(()=>{
      return JSON.stringify('');
    });
    loadFromStorage();

    expect(getClickedVariation()).toEqual('');
    expect(getClickedVariation).not.toEqual(variation1);
    expect(localStorage.setItem).toHaveBeenCalledTimes(0);
    expect(localStorage.getItem).toHaveBeenCalledWith('clickedVariation');
  })
})